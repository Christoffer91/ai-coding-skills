---
name: critique
description: "Use when an idea, plan, prompt, strategy, architecture/product choice, or proposed change needs fair but rigorous pushback before deciding. Use instead of autoreview for non-diff critique, architecture-review for system boundary analysis, risk-assess for go/no-go scoring, and security-review for security-sensitive decisions. Triggers: critique, rubber duck, challenge this, devil's advocate, kritisk blikk, utfordre dette, sanity check, pre-mortem."
---
...
## Boundaries
- Default to read-only reasoning. Do not edit files, run commands, commit, push, install, deploy, publish, or mutate state from this skill alone.
- Do not replace `autoreview` for diff review, `risk-assess` for formal go/no-go scoring, `architecture-review` for system design review, or `security-review` for security-sensitive work.
...
- If you cannot verify a claim in the current context, say what assumption you are making.
...
## Workflow
1. Choose the mode:
   - Use `rubber-duck` when the user is still forming the idea.
   - Use `critique` for most explicit critique requests.
   - Use `hard-challenge` when the user asks to push back hard or the stakes are high.
   - Use `decision` when there are competing options.
...
- Maintenance: Will future Christoffer or future agents understand and sustain it?
...
- Strongest version: Add a React Bits hero text reveal because it improves the first impression without changing the page structure.
- Main challenge: The trade is polish versus motion/load cost. Confirm it works if Lighthouse stays stable and reduced-motion users get readable static text; disconfirm if it delays content or distracts from the CTA.
- Hidden assumption: The hero currently lacks polish more than it lacks message clarity.
- Simpler / safer move: Try one low-cost text effect, not a full animated background.
- Next step: Prototype it behind reduced-motion handling, then compare before/after screenshots and build output.
```
...
- Hidden implementation: making changes or running risky tools under the cover of critique.
...
- Use `security-review` for auth, CORS, secrets, permissions, data exposure, prompt injection, or threat-model concerns.
- Use `frontend-design` or `ux-review` when the critique is about a user-visible surface.
- Use `autoreview` only after there is a concrete diff or branch to review.
