...
3. After the explicit user review command, run `orchestrate-status resume --id <id> --reason "explicit user review resume"`, then set step 5 active. The resume transition clears stale watchdog state while preserving PR/review metadata; a bare `step` is rejected while the run is still in `handoff`. The Live-status rule "emit at each real transition" applies to the review leg too: step 5 `done` after the verdict, step 6 `active`/`done` per fix round, step 7 `active` at the deploy decision — and the review-count metrics below are mandatory, not optional.
